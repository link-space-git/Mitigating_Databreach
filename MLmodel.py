...
class AlertPopup:
    ...
    def __init__(self, parent, alert, detector, mass_activities=None):
        try:
            ...
            # Database logging - completely isolated to prevent crashes
            try:
                self._log_alert_to_database()
            except Exception as db_error:
                print(f"⚠️ Database logging error (non-critical): {db_error}")
            
            # Create popup - this is critical
            self.create_popup()
            ...
            print(f"❌ CRITICAL ERROR in AlertPopup.__init__: {e}")
            import traceback
            traceback.print_exc()

    def _log_alert_to_database(self):
        """Log the alert to MySQL database (non-blocking, safe)."""
        try:
            # Safely import database manager
            try:
                from database_manager import db_manager
            except ImportError:
                print("⚠️ Database manager module not available")
                return
            
            # Check if database is enabled
            if not hasattr(db_manager, 'enabled') or not db_manager.enabled:
                return
            
            # Safely get values with defaults
            username = str(self.alert.get('user', 'unknown'))
            alert_level = str(self.alert.get('alert_level', 'INFO'))
            reason = str(self.alert.get('reason', 'No reason provided'))
            
            # Safely get prediction
            prediction = 'Unknown'
            try:
                if hasattr(self, 'get_prediction'):
                    prediction = str(self.get_prediction())
            except:
                prediction = 'Unknown'
            
            file_count = len(self.mass_activities) if self.mass_activities else 1
            
            # Log to database (this might fail but that's OK)
            ...
            print(f"📝 Alert logged to database: {alert_level} - {reason[:50]}...")
            
        except Exception as e:
            # Don't let database errors affect the popup display
    ...
    def create_popup(self):
        ...
        try:
            ...
            timestamp = self.alert.get('original_timestamp', datetime.now())
            ...
            # Severity
            severity_color = '#ff4757' if self.alert.get('alert_level', '') == 'CRITICAL' else '#ffa502'
            severity_text = f"🔴  {self.alert.get('alert_level', 'UNKNOWN')}"
            ...
            # User
            tk.Label(content_frame, text=f"User: {self.alert.get('user', 'Unknown')}", 
...
class AutomatedAnomalyDetectorGUI:
    ...
    def __init__(self, root):
        ...
        self.model_path = get_resource_path("product/trained_model.pkl")
        self.training_flag_path = get_resource_path("product/training_completed.flag")
        ...
        self.csv_path_var = tk.StringVar(value=get_resource_path(r"log_data/file_activity.csv"))
        self.training_data_path = get_resource_path(r"train/training_baseline_link_only.csv")
    ...
    def _save_trained_model(self):
        """Save the trained model to file for persistence."""
        try:
            import pickle
            model_state = {
                'model': self.detector.model,
                'normal_threshold': self.detector.normal_threshold,
                'anomaly_threshold': self.detector.anomaly_threshold,
                'baseline_trained': self.detector.baseline_trained,
                'training_samples': self.detector.training_samples
            }
            ...
            with open(self.training_flag_path, "w") as f:
    ...
    def _get_timestamp_file_path(self):
        ...
        return get_resource_path("log_data/last_processed.json")
    ...
    def run_training(self, file_path):
        try:
            ...
            # Schedule the completion callback in the main thread
    ...
    def update_progress(self, progress):
        ...
        def _update():
            ...
            self.status_var.set(f"Training: {progress:.1f}%")
...
class AnomalyDetector:
    ...
    def detect_anomalies(self, event):
        ...
        # Handle alert generation for anomalies and critical events
        if result['classification'] in ['ANOMALY', 'CRITICAL']:
            ...
            # DATABASE LOGGING - Non-blocking, wrapped in try-except
            try:
                from database_manager import db_manager
                if hasattr(db_manager, 'enabled') and db_manager.enabled:
                    ...
                    anomaly_score = result['smoothed_score'] if 'smoothed_score' in result else result.get('score', 0)
                    
                    # Log the specific file activity that triggered the anomaly
                    db_manager.log_file_activity(
                        username=username,
                        action=result.get('classification', 'ANOMALY'),
                        file_path=file_path,
                        file_type=file_type,
                        anomaly_score=anomaly_score
                    )
            except Exception as e:
                # Don't let database errors affect the detection
                print(f"⚠️ Database anomaly logging error: {e}")
    ...
    def _initial_detection(self, event):
        ...
        # Non-blocking database logging for initial detection
        try:
            from database_manager import db_manager
            if hasattr(db_manager, 'enabled') and db_manager.enabled:
                username = event.get('user', 'unknown')
            ...
            print(f"⚠️ Database logging error in initial detection: {e}")
