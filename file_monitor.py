...
class FileMonitorEventHandler(FileSystemEventHandler):
    ...
    def should_ignore(self, path):
        ...
        if abs_path.startswith(self.log_dir):
            return True
        
        # Ignore application directory
    ...
    def log_action(self, action, path, dest_path=None):
        """Log file action with automatic IP detection"""
        ...
        # Log to GUI
        if dest_path:
            message = f"[{timestamp}] User: {user} | IP: {ip_address} | {action}: {path} -> {dest_path}"
        else:
            message = f"[{timestamp}] User: {user} | IP: {ip_address} | {action}: {path}"
